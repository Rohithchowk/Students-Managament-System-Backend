...
//     try {
//         // Find all documents with the specified block name
//         const blockData = await BlockData.find({ 'Block': block });
//         if (!blockData || blockData.length === 0) {
//             return res.status(404).json({ error: 'Block not found' });
//         }

//         // Filter out documents that contain the specified department
//         const departmentCategories = [];
//         blockData.forEach(doc => {
//             const departmentData = doc.Department.find(dep => dep.name === department);
//             if (departmentData) {
//                 // Get categories for the department
//                 const categories = Object.keys(doc.toObject()).filter(key => key !== 'Block' && key !== 'Department' && key !== '_id' && key !== '__v');
//                 departmentCategories.push(...categories);
//             }
//         });
...
//         // Return the list of categories for the department
...
router.put('L/IT/labs/:id', async (req, res) => {
    const { id } = req.params;
    const newData = req.body;
